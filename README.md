# Sucuri
> **New:** A windowed tutorial + graph lab is available in `docs/sucuri_workspace_tutorial.html`. This in-progress UI is meant to help explore how Sucuri works and is still actively being developed.
...
This is the current repository for Sucuri, python 2.7.x was dropped, so python **3.14 or newer is required**. The runtime assumes the latest no-GIL-capable interpreter because the scheduler now relies on multithreading rather than multiprocessing.

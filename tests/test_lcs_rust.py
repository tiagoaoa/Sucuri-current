...
class LcsRustIntegrationTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.context = lcs_parallel.LCSContext.from_strings(
            cls.sequence_a, cls.sequence_b, cls.block_size
        )
    ...
    def run_backend(self, workers: int, compute_fn, repeats: int = 2) -> tuple[int, float]:
        ...
        for _ in range(repeats):
            ...
            score = lcs_parallel.compute_lcs(self.context, workers, compute_fn)
    ...
    def test_rust_matches_python_results(self) -> None:
        for workers in (1, 4, 8):
            python_score, _ = self.run_backend(workers, lcs_parallel.lcs_block_python)
            rust_score, _ = self.run_backend(workers, lcs_parallel.lcs_block_rust)
            self.assertEqual(
                python_score,
                rust_score,
    ...
    def test_rust_speedup_scaling(self) -> None:
        ...
        self.run_backend(1, lcs_parallel.lcs_block_python)
        self.run_backend(1, lcs_parallel.lcs_block_rust)
        ...
        for workers in worker_counts:
            _, python_time = self.run_backend(workers, lcs_parallel.lcs_block_python)
            _, rust_time = self.run_backend(workers, lcs_parallel.lcs_block_rust)
